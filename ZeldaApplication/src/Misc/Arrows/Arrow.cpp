...
Arrow::Arrow(uint32_t _id, AnimationSheet* _sheet, Ref<Scene> _scene)
{
	...
	EmplaceAnimation(new FrameRangeAnimation("frame_arrow_left", 0, m_films["arrow_left"]->GetTotalFrames(), 0, 300, 12 * 16, 150));
	EmplaceAnimation(new FrameRangeAnimation("frame_arrow_right", 0, m_films["arrow_right"]->GetTotalFrames(), 0, 300, 12 * 16, 150));
	EmplaceAnimation(new MovingAnimation("mov_moving", 0, 0, 0, 20));
	EmplaceAnimation(new MovingAnimation("mov_gravity", 0, 0, 0, 6));
